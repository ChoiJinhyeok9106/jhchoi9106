 ...
 *  
 * @Description
 * 일반 이미지 게시판 리스트 위젯 (720px)
 * 
 * @Syntax 
 * 	$.get('/widget/template/community/normal-img-list.js', function(response){
 *		$('head').append(response); 
 *	});
 * 	<component :is="'normal-img-list'" :appid="appid" :widgetseq="widgetseq"></component>
 * 
 * @Parameters
 *  appid : 앱 아이디
 *  widgetseq : 위젯 번호
 ********************************************************************************************/ 
Vue.component('normal-img-list', {
		...
		widgetRefresh: function(){
			try{
				...
				if(!isNull(this.param.BOARD_SEQ)){
					var self = this;
					var returnVal = boardApiApi.getWidgetList(this.param.BOARD_SEQ, 3, false);
	...
	template:`
		<article class="widget_box" ref="mainListLoading" v-if="thumbnail != true">
            ...
            <section class="content_box">
                <div class="inner">
                    <div class="widget-type03" v-if="widgetData.getRowCount() > 0">
                        <ul>
                            <li v-for="(item, index) in widgetData.data">
                            	<a href="#!" @click="appOpen(item.POST_NO, item.BOARD_SEQ, item.BOARD_GB)">
	                                <span class="thumb-box"><img :src="item.PREV_IMG_PATH" alt="" /></span>
	                                <div class="cont-box">
	                                    <p class="tit">{{item.TITLE}}</p>
	                                    <p class="txt" v-html="item.CONTENTS"></p>
	                                    <span class="etc icon-eye">{{item.SEARCH_CNT}}</span><span class="etc icon-realtime">{{item.INSERT_DTTM | capitalize}}</span>
	                                </div>
	                        	</a>
                            </li>
                        </ul>
        ...
        <article class="widget_box" v-else-if="thumbnail == true">
            ...
            <section class="content_box">
                <div class="inner">
                    <div class="widget-type03">
                        <ul>
                            <li><a href="#!">
                                <span class="thumb-box"><img src="/widget/images/thumb_img01.jpg" alt="" /></span>
                                <div class="cont-box">
                                    <p class="tit"></p>
                                    <p class="txt"></p>
                                    <span class="etc icon-eye">687</span><span class="etc icon-realtime">2019. 01. 02</span>
                                </div></a>
                            </li>
                            <li><a href="#!">
                                <span class="thumb-box"><img src="/widget/images/thumb_noimg01.png" alt="" /></span>
                                <div class="cont-box">
                                    <p class="tit"></p>
                                    <p class="txt"></p>
                                    <span class="etc icon-eye">687</span><span class="etc icon-realtime">2019. 01. 02</span>
