 ...
 * 
 * @Syntax 
 * 	$.get('/widget/template/community/market-list.js', function(response){
 *		$('head').append(response); 
 *	});
 * 	<component :is="'market-list'" :appid="appid" :widgetseq="widgetseq"></component>
 * 
 * @Parameters
 *  appid : 앱 아이디
 *  widgetseq : 위젯 번호
...
Vue.component('market-list', {
	...
	methods: {
		...
		widgetRefresh: function(){
			try{
				...
				if(!isNull(this.param.BOARD_SEQ)){
					var self = this;
					var returnVal = boardApiApi.getMarketList(5, false);
		...
		appOpen: function(pPostNo, pBoardSeq, pBoardGb,menuCd){
			...
			if(pBoardSeq==9999) {
				...
				url += '/community/sub/sub03_03_list.html?&postNo='+pPostNo+'&mainYn=Y';
                    ...
                    <div class="widget-type01" v-if="widgetData.getRowCount() > 0">
                        <ul>
                            <li v-for="(item, index) in widgetData.data">
                            	<a href="#!" @click="appOpen(item.POST_NO, item.BOARD_SEQ, item.BOARD_GB,param.MENU_CD)">
                            	    <span class="date-box img-box" v-if="item.PROD_GB == '1'"><img :src="item.PREV_IMG_PATH"></span>
	                                <span class="date-box" v-if="item.PROD_GB == '2'"><span class="big">{{item.INSERT_DTTM.substring(6,8)}}</span>{{item.INSERT_DTTM.substring(0,4)}}. {{item.INSERT_DTTM.substring(4, 6)}}</span>
	                                <div class="cont-box">
	                                	<p class="writer">{{item.BOARD_NM}}</p>
	                                    <p class="tit">{{item.TITLE}}</p>
	                                    <p class="txt">{{item.CONTENTS}}</p>
	                                    <span class="etc icon-eye" v-if="item.PROD_GB == '2'">{{item.SEARCH_CNT}}</span><span class="etc icon-realtime">{{item.INSERT_DTTM | capitalize}}</span><span class="writer">{{item.JAKSEONGJA_NM}}</span>
	                                </div>
                                </a>
                            </li>
                        </ul>
